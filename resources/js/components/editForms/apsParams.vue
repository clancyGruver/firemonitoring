<template>
	<modal name="aps-params" transition="pop-out" height="auto">
		<div class="mb-4">
			<div class="card-body">
				<div class="p-0">
  	...
  	name: "aps-params",
		...
		methods: {
			save(){
				if(!this.check())
					return false;
				this.$store.dispatch('UPDATE_OBJECT_DEVICE_PARAMS', {type:'aps', data: this.deviceData});
				this.cancel();
			},
			cancel () {
      	this.$modal.hide("aps-params");
			},
			check(){
				let res = true;
				this.errors = [];
